...
class Rudder():
    def __init__(self, eta2, V2, phid, c_dir, c_speed):
        self.eta2 = eta2
        self.V2 = V2
        self.phid = phid
        self.c_dir = c_dir
    ...
    def angle(self):
        phit = self.eta2.item(3)
        rudder_controllor = PID(5, 15, 0.001, -0.78, 0.78, -0.5, 0.5)
        return rudder_controllor.update(phit, self.phid)
    ...
    def force(self):
        ...
        CL = 1.8 * pi * lamda * self.angle() / (1.8 + cos(chi) * m) + CDC * self.angle() ** 2 / lamda
